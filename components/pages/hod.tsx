...
import { UserCog, SearchX, Layers } from "lucide-react"
...
export function HodDashboard() {
  ...
  const filteredLeaves = useMemo(() => {
    return leaves.filter(l => {
      ...
      const statusToMatch =
        filterStatus === "pending" ? "staff_approved" :
        filterStatus === "approved" ? "hod_approved" :
        filterStatus
      const matchesFilter = filterStatus === "all" || l.status === statusToMatch
  ...
  const stats = useMemo(() => {
    return {
      total: leaves.length,
      pending: leaves.filter(l => l.status === "staff_approved").length,
      approved: leaves.filter(l => l.status === "hod_approved").length,
  ...
  return (
    <PageLayout 
        title="HOD Approval Dashboard" 
        description="Final review and sign-off for staff-approved leave requests"
    >
      <div className="space-y-8 max-w-6xl mx-auto">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 border-b border-border/50 pb-4">
             <UserCog className="h-5 w-5 text-primary opacity-50" />
             <h2 className="text-xl font-black italic tracking-tighter text-foreground uppercase opacity-80">
                {filterStatus === "pending" ? "Final Sign-off Required" : "Processed Applications"}
             </h2>
          </div>
          ...
          {filteredLeaves.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 px-4 bg-white rounded-3xl border-2 border-dashed border-border/50 group hover:border-primary/30 transition-colors">
               ...
               <p className="text-xl font-black text-foreground italic tracking-tight">Zero pending applications!</p>
               <p className="text-sm text-muted-foreground font-medium mt-1">Excellent productivity today.</p>
            </div>
          ) : (
